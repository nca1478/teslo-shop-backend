...
export class PlaceOrderUseCase {
    ...
    async execute(request: PlaceOrderRequest): Promise<Order> {
        ...
        for (const item of items) {
            ...
            if (!product) {
                throw new ValidationDomainException(`Product ${item.productId} not found`);
            }
        ...
        const order = await this.orderRepository.createWithAddress(orderData, request.address);
