...
export class PayPalCheckPaymentUseCase {
    ...
    async execute(request: PayPalCheckPaymentRequest): Promise<Order> {
        ...
        const paymentDetails = await this.paypalService.verifyPayment(paypalTransactionId);

        if (paymentDetails.status !== 'COMPLETED') {
            throw new ValidationDomainException('Payment not completed');
        }

        const order = await this.orderRepository.findByTransactionId(paypalTransactionId);
        ...
        if (!isTestTransaction && Math.abs(paymentAmount - order.total) > 0.01) {
            throw new ValidationDomainException('Payment amount does not match order total');
        }
